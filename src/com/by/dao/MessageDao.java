...
public class MessageDao extends ConnectionFactory {
	...
	public static void main(String[] args) {
		...
		comment.setReplaydate(20151021);
		comment.setReplycontent("那就不回复了");
		MessageDao messageDao = new MessageDao();
		
//		messageDao.insertMessageObject(comment);
//		messageDao.updateMessageByTourist(comment);
//		System.out.println(messageDao.getAllCommentByUserId("123"));
		System.out.println(messageDao.getAllMessageByTouristId("13333"));
	...
	public boolean insertMessageObject(Message message) {
		...
		String sql = "INSERT INTO message(identify,userid, touristid, content,commentdate,phonenumber) VALUES (?,?,?,?,?,?)";
...
//				pstmt.executeBatch();
	...
	public ArrayList<Message> getAllMessageByTouristId (String identify) {
		...
		String sql = "select * from message where touristid = "+identify;
...
//				System.out.println(touristList);
	 ...
	 */
		public Boolean updateMessageByTourist(Message message) {
			Boolean status = true;
			Connection connect = null;
			String sql = "update message set userid = ?,touristid = ?,replycontent = ? ,replydate = ? where identify = ?";
			PreparedStatement pstmt = null;
			System.out.print(sql);
			connect = createConnect();
			ResultSet rs = null;
